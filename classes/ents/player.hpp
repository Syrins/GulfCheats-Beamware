...
public:
	...
	Player(uint64_t _ent)
	{
		...
		this->steam_id = (rust->mem->Read<int>(this->ent + 0x20) + 0x0110000100000000);
	...
	bool Update()
	{
		...
		// check player health
		if (this->health < 1 || this->health > 100) return false;
...
private:
	...
	{
		std::wstring name = rust->mem->ReadUnicode(rust->mem->Read<uint64_t>(this->ent + offsets->playerDisplayName) + 0x14);
