...
void Menu::Init()
{
	...
	const auto rage_tab = new C_Tab(this, 0, "e", std::string(skCrypt("aim")), { {"-", 0, 3}, {"a", 1, 3}, {"b", 2, 3}, {"c", 3, 3}, {"d", 4, 3}, {"e", 5, 3}, {"f", 6, 3}, {"g", 7, 3} }); {
		mtv->tmenu.tab_sub = 0;
		...
		const auto legit_aimbot = new C_GroupBox(GROUP_LEFT, 8, "t"); {
			...
			//const auto psilent = new C_CheckBox("psilent", &settings->aimbot.psilent[0]);
			//new C_Spacer(psilent, { 0, 20 });
		}
		...
		for (int i = 0; i < 7; i++)
		{
			mtv->tmenu.tab_sub = i + 1;

			const auto legit_aimbot = new C_GroupBox(GROUP_LEFT, 8, "t"); {
				new C_CheckBox(std::string(skCrypt("aimbot enabled")), &settings->aimbot.enabled[i + 1]);

				const auto fov_slider = new C_SliderInt(std::string(skCrypt("field of view")), &settings->aimbot.fov[i + 1], 0, 180, "°", {});
				new C_Spacer(fov_slider, { 0, 5 });

				const auto bone_selection = new C_Dropdown(std::string(skCrypt("bone selection")), &settings->aimbot.bone_selection[i + 1], { "head", "body", "cock", "closest to cursor" });
				new C_Spacer(bone_selection, { 0, 15 });

				//const auto psilent = new C_CheckBox("psilent", &settings->aimbot.psilent[i + 1]);
				//new C_Spacer(psilent, { 0, 20 });
			}

			const auto legit_antiaim = new C_GroupBox(GROUP_RIGHT, 8, "t"); {
				new C_CheckBox(std::string(skCrypt("smoothing")), &settings->aimbot.smoothing[i + 1]);

				const auto smoothing_slider = new C_SliderInt(std::string(skCrypt("smoothing amount")), &settings->aimbot.smoothing_amount[i + 1], 0, 4, "°");
				new C_Spacer(smoothing_slider, { 0, 5 });

				const auto anti_recoil = new C_CheckBox(std::string(skCrypt("anti-recoil")), &settings->aimbot.anti_recoil[i + 1]);
				new C_Spacer(anti_recoil, { 0, 15 });

				const auto prediction_toggle2 = new C_CheckBox(std::string(skCrypt("prediction")), &settings->aimbot.prediction[i + 1]);
				new C_Spacer(prediction_toggle2, { 0, 20 });
			}
		}
	}
