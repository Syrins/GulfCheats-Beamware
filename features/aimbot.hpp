...
public: // public methods for this class
	...
	Vector3 Prediction(Vector3 playerPos, Vector3 localPos)
	{
		...
		// return the result
		//Normalize(result.x, result.y, result.z);
		return result;
	}
	...
	void DoAimbot(int weapon_type)
	{
		...
		if (!((settings->aimbot.enabled[0]) ? (settings->aimbot.enabled[0]) : (settings->aimbot.enabled[weapon_type]))) return;
		...
		switch (((settings->aimbot.bone_selection[0]) ? (settings->aimbot.bone_selection[0]) : (settings->aimbot.bone_selection[weapon_type])))
		{
		case 0:	// head bone
		...
		if ((settings->aimbot.anti_recoil[0]) ? (settings->aimbot.anti_recoil[0]) : (settings->aimbot.anti_recoil[weapon_type]))
		{
		...
		if ((settings->aimbot.smoothing[0]) ? (settings->aimbot.smoothing[0]) : (settings->aimbot.smoothing[weapon_type]))
		{
			// smooth aimbot
			this->SmoothAim(v2_aim, ((settings->aimbot.smoothing_amount[0]) ? (settings->aimbot.smoothing_amount[0]) : (settings->aimbot.smoothing_amount[weapon_type])));
		}
		else
		{
			// smooth aimbot
	...
	void FindTarget()
	{
		while (1)
		{
			// sleep to calm down CPU
			SleepEx(5, false);

			if (!globals->local_player.ent) continue;
			...
			if (((settings->aimbot.enabled[0]) ? (settings->aimbot.enabled[0]) : (settings->aimbot.enabled[weapon_type])) && GetAsyncKeyState(settings->aimbot.keybind))
			{
				// holds the closest fov
				float closest_fov = (settings->aimbot.fov[0] != 0) ? (settings->aimbot.fov[0]) : (settings->aimbot.fov[weapon_type]);
				...
				std::map<uint64_t, Player> players = globals->players;
				...
				{
					if (!globals->local_player.ent) continue;
					if ((settings->aimbot.psilent[0]) ? (settings->aimbot.psilent[0]) : (settings->aimbot.psilent[weapon_type])) continue;
