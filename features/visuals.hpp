...
namespace visuals {
	...
	void DrawBasic(T entity, std::string name, bool b_distance, bool b_name, bool b_box, bool b_flags, ImVec4* color)
	{
		...
		if constexpr (std::is_same<T, Stash>::value)
		{
			ImVec2 tsize = ImGui::CalcTextSize("HIDDEN", NULL);

			// sv_show_open_stash_bypass_EAC 1
			if (b_flags && entity.hidden)
				drawing->text({ ent_pos.x - (tsize.x / 2), ent_pos.y + (tsize.y + 1.f) }, "HIDDEN", { color->x, color->y, color->z, settings->visuals.font_fading ? (float)((1 - (0.005 * entity.Distance(&globals->local_player)))) : 1.f }, true);
		}
	}
	...
	void Draw()
	{
		...
		ImGui::Begin("main", NULL, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_None | ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoFocusOnAppearing);
		{
			if (!globals->has_launched) {
				...
				std::map<uint64_t, Ore> ores = globals->ores.map;
				for (auto& entry : ores) {
